...
export class AIService {
  ...
  async semanticSearch(
    ...
    const modelENV = this.getModelENV(inputModel);
    this.logger.log('AI Context', context);

    try {
      const model = this.AI.chat(modelENV);
      const response = await model.doStream({
        providerMetadata: {
          openrouter: {
            tranforms: ['middle-out'],
          },
        },
  ...
  async analyzeDocs(fileInfo: FileInfo) {
    ...
    const model = this.AI(this.configService.getOrThrow('DEEPSEEK_MODEL'));
    try {
