...
export class AIService {
  ...
  async semanticSearch(
    ...
    try {
      ...
      this.logger.error('[SemanticSearch]', (error as Error).message);
      throw error;
    }
  }
  ...
  async analyzeDocs(fileInfo: FileInfo) {
    ...
    try {
        ...
        messages: [
          {
            ...
            content: `You are an assistant that analyzes documents and produces clear,
              ...
              Write the summarized text as a markdown format without any additional texts`,
          },
          {
            role: 'user',
      ...
      this.logger.error('[analyzeDocs]', (error as Error).message);
      this.handleErrorResponse(error as APIError);
    }
  }

  private handleErrorResponse(error: APIError) {
    if (error?.status === 400) {
      throw new BadRequestException(
        'The document could not be processed. Please check the file format or content.',
      );
    }
    if (error?.status === 429) {
      throw new ServiceUnavailableException(
        'The analysis service is currently overloaded. Please try again later.',
      );
    }
    if (error?.status === 500) {
      throw new InternalServerErrorException(
        'The analysis service encountered an unexpected error. Please retry in a moment.',
      );
    }
    throw new InternalServerErrorException(
  ...
  async getFileContent(userId: number, fileId: number) {
    ...
    try {
      ...
      this.logger.log('[getFileContent]', (error as Error).message);
      throw new InternalServerErrorException(
